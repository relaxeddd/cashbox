...
import ru.cashbox.android.databinding.FragmentLoginEmployeeBinding
import ru.cashbox.android.model.EventType

class FragmentLoginEmployee : FragmentBase<ViewModelLoginEmployee, FragmentLoginEmployeeBinding>() {

    override fun getLayoutResId() = R.layout.fragment_login_employee
    ...
    override fun onNavigationEvent(type: EventType, args: Bundle?) {
        when (type) {
            EventType.NAVIGATION_LOGIN_EMPLOYEE_TO_CASH -> {
                navigationController.navigate(R.id.action_navigation_login_employee_to_navigation_cash)
            }

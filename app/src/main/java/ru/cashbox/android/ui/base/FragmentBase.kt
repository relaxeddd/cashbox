...
import ru.cashbox.android.ui.ActivityMain
...
abstract class FragmentBase<VM : ViewModelBase, B : ViewDataBinding> : Fragment() {
    ...
    open fun onBackPressed() = if (viewModel.isLoadingVisible.value == true) {
    ...
    protected open fun onNavigationEvent(type: EventType, args: Bundle?) {
        when (type) {
            EventType.NAVIGATION_UNAUTHORIZED -> {
                viewModel.onUnauthorized()
                navigationController.navigate(R.id.action_global_fragmentLoginEmployee)
            }

...
import ru.cashbox.android.model.repositories.RepositoryController
...
abstract class ViewModelBase(app: App) : AndroidViewModel(app) {
    ...
    protected fun showErrorIfIncorrect(response: Response<*>) {
        if (!response.isSuccessful) {
            ...
            errorStr = if (serverStr?.isNotEmpty() == true) {
                JSONObject(serverStr).optString(MESSAGE)
            } else {
                getErrorStringByCode(response.code())
            }
    ...
    fun onUnauthorized() {
        RepositoryController.onUnauthorized()
    }
